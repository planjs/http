...
export class MYAdapter implements Adapter {
  ...
  constructor(req: Request) {
    ...
    this.response = new Promise<Response>((resolve, reject) => {
      const supportMethod: my.RequestMethod[] = ['GET', 'POST', 'PUT', 'DELETE'];
      const method = RequestMethod[req.method].toUpperCase() as my.RequestMethod;
      if (!supportMethod.includes(method)) {
        throw new Error('MYadapter Does not support secondary requests');
      }
      if (req.headers == null) {
        req.headers = new Headers();
      }
      const headers: { [key: string]: string } = {};
      req.headers.forEach((values, name) => (headers[name!] = values.join(',')));
      ...
      if (req.url && req.url.slice(0, 6) !== 'https:') {
        ...
        if (process.env.NODE_ENV === 'production') {
          my.showToast({ title: 'Please check the url WXAdapter must be https' });
        }
      }
      ...
      const requestTask = my.request({
        ...
        data: req.getBody(),
        timeout: req.timeOut !== null ? 30000 : req.timeOut!,
        success(res) {
...
export class ConnectionMY implements ConnectionAdapter {
  createConnection(req: Request) {
    return new MYAdapter(req);
  }
}
